...
export interface CreateEventPayload {
  ...
  recurrence_end_at?: string // ISO8601
}
```
...
- **模式**：支援「新增」與「編輯」模式。編輯模式透過 URL query `id` 觸發。
- **時間校驗**：
  - `end_at` 必須晚於 `start_at`
  - 活動跨度不得超過 7 天
  - `recurrence_end_at` 若存在，不得早於 `start_at`
...
1. **Header**（固定頂部）
   - 左：`arrow_back` 返回按鈕 + 「新增活動/編輯活動」標題（`#2b9dee`）
   - 右：「儲存」按鈕（`#2b9dee`）
...
- 共 5 組 popup（startDate, startTime, endDate, endTime, recurrenceEndDate）
...
| 檔案 | 用途 |
|------|------|
| `app/types/event.ts` | 定義 `Event` 與 `CreateEventPayload` |
| `app/composables/useCalendarEditor.ts` | 表單狀態、驗證、儲存、刪除邏輯 |
| `app/pages/calendar/editor.vue` | 編輯頁面 UI |
| `app/pages/calendar/index.vue` | 「新增活動」按鈕與活動詳情跳轉 |
| `app/services/eventService.ts` | 處理 CRUD API 調用 |
...
// 回傳值
{
  formData,                // ref: 表單狀態物件 (含 recurrenceEndDate)
  ...
  initForm(date?: string), // 初始化新增模式預設值
  initEditor(),            // 根據路由 query 初始化 (新增或編輯)
  validateForm(),          // 回傳 { valid: boolean, error?: string }
  saveEvent(),             // 儲存 (新增或更新) 至 Supabase
  deleteEvent(),           // 刪除活動
...
- 新增入口：`/calendar` 頁面的「新增活動」按鈕 (`query: { date: selectedDate }`)
- 編輯入口：`/calendar` 活動詳情點擊「編輯」(`query: { id: eventId }`)
- 返回：Header 返回箭頭 → `router.back()`
- 儲存/刪除成功：`router.push('/calendar')`
